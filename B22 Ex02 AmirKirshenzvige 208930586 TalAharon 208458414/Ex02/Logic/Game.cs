...
namespace Ex02.Logic
{
    ...
    public class Game
    {
        ...
        public void Run()
        {
            UI.UserInteraction.Print_WelcomeToGame();
        ...
        private void SetGame()
        {
            ...
            //m_GameBoard = new BoardUtilities(boardType);
        ...
        private void PlayGame()
        {
            ...
            while (!m_IsAllGameFinished)
            {
                ...
                UI.UserInteraction.Print_SingleGameFinished();
                UpdateScore();
        ...
        {
            UI.UserInteraction.Print_AllGameFinished();

            if (m_Player1.Score > m_Player2.Score)
            {
                UI.UserInteraction.Print_AllGameWinner(m_Player1.Name, m_Player1.Score);
                UI.UserInteraction.Print_AllGameLoser(m_Player2.Name, m_Player2.Score);
            }
            else if (m_Player1.Score < m_Player2.Score)
            {
                UI.UserInteraction.Print_AllGameWinner(m_Player2.Name, m_Player2.Score);
                UI.UserInteraction.Print_AllGameLoser(m_Player1.Name, m_Player1.Score);
            }
            else // TIE
            {
                UI.UserInteraction.Print_AllGameFinishedWithATie();
            }

            UI.UserInteraction.Print_ExitGame();
        }
        ...
        private void SingleGameFinishedMessage(eGameStatus i_FinishMode)
        {
            ...
            {
                UI.UserInteraction.Print_SingleGameWinner(m_Player1.Name, player1SingleGameScore);
                UI.UserInteraction.Print_SingleGameLoser(m_Player2.Name, player2SingleGameScore);
            }
            else if (i_FinishMode == eGameStatus.SecondPlayerWin)
            {
                UI.UserInteraction.Print_SingleGameWinner(m_Player2.Name, player2SingleGameScore);
                UI.UserInteraction.Print_SingleGameLoser(m_Player1.Name, player1SingleGameScore);
            }
            else if (i_FinishMode == eGameStatus.Tie)
            {
                UI.UserInteraction.Print_SingleGameFinishedWithATie();
            }

            UI.UserInteraction.Print_PlayersTotalScore(m_Player1.Name, m_Player2.Name, m_Player1.Score, m_Player2.Score);
        }
